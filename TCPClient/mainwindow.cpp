...
void MainWindow::on_pushButton_clicked()
{
    ...
    string cipher=encrypt_aes(key,utf8_text); //Encryption part
    string a=convert_to_hex(cipher); //Convert hex to string
    QString qstr = QString::fromStdString(a);

    QByteArray byteArray;
    QBuffer buffer(&byteArray);
    image.save(&buffer, "PNG");
    QString iconBase64 = QString::fromLatin1(byteArray.toBase64().data());// Convert image to base64
    ...
    QJsonObject mainObject;  //Make Json 
    ...
    tcpClient->sendMessage(jsonString.toUtf8()); //Send message from client to server 
}
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    if(QString::compare(filename,QString()) !=0)
    {
        ...
        if(valid)
        {
            ...
            ui->label->setPixmap(QPixmap::fromImage(image));//Select Photo from pc
        }
